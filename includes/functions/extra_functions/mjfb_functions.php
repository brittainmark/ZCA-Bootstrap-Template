...
/*
 * Function to set the cheapest shipping excluding surface mail options.

function mjfb_cheapest_shiping (){
	$modules = MODULE_SHIPPING_INSTALLED;
	...
	if (!empty($modules)){
		...
		for ($i=0; $i<$size; $i++) {
			if (is_array($cheapest)) {
				...
				and substr($rates[$i]['module'],0,4) != 'rmsm') {
					echo"<br /> module = ".$rates[$i]['module'];
					$cheapest = $rates[$i];
				}
			} else {
				if ($rates[$i]['module'] != 'storepickup' and substr($rates[$i]['module'],0,4) != 'rmsm') {
					$cheapest = $rates[$i];
				}
			}
		}
